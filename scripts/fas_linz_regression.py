...
def write_outputs(
    ...
    # Persist per-participant held-out predictions (long format) so the
    # predicted-vs-actual figure can be regenerated from data, not the PNG.
    paths["predictions"] = output_dir / "linz_fas_predictions.csv"
    ...
    for r in results:
        ...
        ppath = output_dir / f"linz_fas_predictions_{slug}.csv"
        pd.DataFrame({
